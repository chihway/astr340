...
class AsinhMetric:
    ...
    def __call__(self, cells, features, errors):
        ...
        # import sys
        # # Some condition or code
        # if np.any(np.isnan(vn)):
        #     print('helloooooo')
        #     print('np.any(np.isnan(cells) ', np.any(np.isnan(cells)))
        #     print('np.any(np.isnan(ee) ', np.any(np.isnan(ee)))
        #     print('np.any(np.isnan(vn) ', np.any(np.isnan(vn)))
        #     sys.exit("Stopping the script due to some_condition")     
